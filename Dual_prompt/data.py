...
def convert_to_feature(raw, tokenizer):
    ...
    tokens_id = tokenizer(line, truncate=True)  # truncate=True 自动截断
    return tokens_id
...
def get_paths(path, name='coco', use_restval=True):
    ...
    if 'coco' == name:
        ...
        #ids['val'] = np.load(os.path.join(capdir, 'coco_train_ids.npy'))[:1000]
        ids['val'] = np.load(os.path.join(capdir, 'coco_test_ids.npy'))[:25000]
...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, root, json, max_len, ids=None,split=None):
        """
        Args:
        ...
        self.blip_processor = AutoProcessor.from_pretrained("Salesforce/blip2-flan-t5-xl")
        ...
        if ids is None:
            ...
            self.ids = ids  # [40987,36541,.....]是一个列表存了专门的测试样本id
        ...
        if isinstance(self.ids, tuple):
            ...
            self.bp = len(self.ids)  # vla为例:5000
    ...
    def __getitem__(self, index):
        ...
        qusetion_global = 'Question: What does the picture convey as a whole? Answer:'
        qusetion_background = 'Question: What background information is contained in this image? Answer:'
        question_entries = 'Question: What entities are included in the image? Answer:'

        '======================针对不同提问对图像进行处理=============================='
        image_blip_global = self.blip_processor(image, qusetion_global,return_tensors="pt")
        ...
        image_blip_global_pixel_values = image_blip_global['pixel_values']
        image_blip_global_input_ids=image_blip_global['input_ids']
        image_blip_global_attention_mask=image_blip_global['attention_mask']

        image_blip_background_pixel_values = image_blip_background['pixel_values']
        image_blip_background_input_ids=image_blip_background['input_ids']
        image_blip_background_attention_mask=image_blip_background['attention_mask']

        image_blip_entries_pixel_values = image_blip_entries['pixel_values']
        image_blip_entries_input_ids = image_blip_entries['input_ids']
        image_blip_entries_attention_mask = image_blip_entries['attention_mask']
        ...
        if is_clip:
            ...
            return (image,caption,
                    image_blip_global_pixel_values,image_blip_global_input_ids,image_blip_global_attention_mask,
                    image_blip_background_pixel_values, image_blip_background_input_ids, image_blip_background_attention_mask,
                    image_blip_entries_pixel_values, image_blip_entries_input_ids, image_blip_entries_attention_mask,
                    input_ids, index)
        else:
            pass
        ...
        if is_clip:
            input_ids = convert_to_feature(caption, self.tokenizer)
            input_ids = torch.as_tensor(input_ids, dtype=torch.long)
            return image,image_blip_global,image_blip_background,image_blip_entries,input_ids, index
        else:
            pass'''
    ...
    def get_raw_item(self, index):
        if index < self.bp:  # index<= 5000
            ...
            coco = self.coco[1]
            root = self.root[1]
        ann_id = self.ids[index]  # self.ids是一个包含5000个样本id的列表
        caption = coco.anns[ann_id]['caption']  # 取得对应id的文本
        img_id = coco.anns[ann_id]['image_id']  # 取得对应id的图像id
...
class FlickrDataset(data.Dataset):
    ...
    def __init__(self, root, json, split, vocab, max_len):
    ...
    def __getitem__(self, index):
        ...
        images = Image.open(os.path.join(root, path)).convert('RGB')

        '======================针对不同提问对图像进行处理=============================='
        image_blip=self.blip_processor(images,return_tensors="pt")
        image_blip_background = self.blip_processor(images,qusetion_background, return_tensors="pt")
        image_blip_entries=self.blip_processor(images,question_entries,return_tensors='pt')
        ...
        if self.transform is not None:
            image_trans = self.transform(images)
        #image_blip=self.blip_processor(image)
        # Convert caption (string) to word ids.
        is_clip = True
        if is_clip:
            input_ids = convert_to_feature(caption, self.tokenizer)
            input_ids = torch.as_tensor(input_ids, dtype=torch.long)
            return image_trans,image_blip,image_blip_background,image_blip_entries, input_ids, index
...
def collate_fn_bert(data):
    ...
    #image, images,images_background,images_entries, input_ids, ids = zip(*data)
    image,caption,image_blip_global_pixel_values, image_blip_global_input_ids, image_blip_global_attention_mask,image_blip_background_pixel_values, image_blip_background_input_ids, image_blip_background_attention_mask,image_blip_entries_pixel_values, image_blip_entries_input_ids, image_blip_entries_attention_mask,input_ids, index =zip(*data)
    input_ids = torch.stack(input_ids, 0)
    input_ids = torch.squeeze(input_ids, dim=1)
    image = torch.stack(image, dim=0)

    image_blip_global_pixel_values=torch.cat(image_blip_global_pixel_values,dim=0)
    image_blip_global_input_ids=torch.cat(image_blip_global_input_ids,dim=0)
    image_blip_global_attention_mask=torch.cat(image_blip_global_attention_mask,dim=0)

    image_blip_background_pixel_values = torch.cat(image_blip_background_pixel_values, dim=0)
    image_blip_background_input_ids = torch.cat(image_blip_background_input_ids, dim=0)
    image_blip_background_attention_mask = torch.cat(image_blip_background_attention_mask, dim=0)

    image_blip_entries_pixel_values = torch.cat(image_blip_entries_pixel_values, dim=0)
    image_blip_entries_input_ids = torch.cat(image_blip_entries_input_ids, dim=0)
    image_blip_entries_attention_mask = torch.cat(image_blip_entries_attention_mask, dim=0)

    #blip2_images = torch.cat(images, dim=0)
    #blip2_images=images
    #blip2_images_background=torch.cat(images_background,dim=0)
    #blip2_images_background = images_background
    #blip2_images_entries=torch.cat(images_entries,dim=0)
    #blip2_images_entries=images_entries
    ids = np.array(index)
    return image,caption,image_blip_global_pixel_values, image_blip_global_input_ids, image_blip_global_attention_mask,image_blip_background_pixel_values, image_blip_background_input_ids, image_blip_background_attention_mask,image_blip_entries_pixel_values, image_blip_entries_input_ids, image_blip_entries_attention_mask,input_ids, ids
