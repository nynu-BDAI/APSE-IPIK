...
class CustomTextEncoder(torch.nn.Module):
    ...
    def forward(self, text_feature, text_prefix, enable_pos_emb=True):
        """The forward function to compute representations for the prompts.
        ...
        x = (
            input_transformer_features + self.positional_embedding.type(self.dtype)
            if enable_pos_emb
            else input_transformer_features
        )
        x = x.permute(1, 0, 2)
        # log.info(f'DEVICE: {type(self.device)}')
