...
class DoubleEncoder(pl.LightningModule):
    def __init__(self, config):
        ...
        #self.align_model=MLP(input_dim=512,hidden_dim=256,output_dim=512)
        ...
        #self.image_proj_back = self.model.visual.proj

    def freeze_clip_parameters(self):
        for name, param in self.model.named_parameters():
            if param.requires_grad is True:
                print(f'{name}_grad is erro_require! ')
                param.requires_grad=False
                print(f'now the {name}_grad is {param.requires_grad}')
    ...
    def infer1(self, batch, image_token_type_idx=1, img=None):
        ...
        '''aligned_image_features = self.align_model(image_output1)
        criterion =CosineSimilarityLoss()
        align_loss = criterion(aligned_image_features, text_output1)'''

        text_output1=torch.unsqueeze(text_output1,dim=1)
        image_output1=torch.unsqueeze(image_output1,dim=1)
        #aligned_image_features=torch.unsqueeze(aligned_image_features,dim=1)
        combined_text_input=torch.cat([specialized_t_prompt,general_t_prompt,text_output1],dim=1)
        #combined_text_input = torch.cat([text_output1, specialized_i_prompt, general_i_prompt], dim=1)
