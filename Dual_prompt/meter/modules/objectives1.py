...
import torch.nn.functional as F
from my_HAT_PL.evaluation1 import i2t_SCAN, t2i_SCAN,i2t_SCAN_NN,t2i_SCAN_NN
import sys
...
def uni_score(images,captions):
    ...
    return similarities
def compute_SCAN(im, s, margin, direction):
...
def compute_irtr_my(pl_module, batch):
    ...
    #input_id:58*1*77
    #images,blip2_images,blip2_images_background,blip2_images_entries,input_ids,ids = batch #dataloader返回值，coll fn返回
    images,caption,image_blip_global_pixel_values, image_blip_global_input_ids, image_blip_global_attention_mask, image_blip_background_pixel_values, image_blip_background_input_ids, image_blip_background_attention_mask, image_blip_entries_pixel_values, image_blip_entries_input_ids, image_blip_entries_attention_mask,input_ids,ids=batch
    #print("Calling infer method")
    ...
    image_feats = infer['image_output_clip']  # (B 49 H) (when infer use befor pool)  after pool:  (B H)
    text_feats = infer['text_output_clip']  # (B 32 H)#                                           (B H)
    image_feats_blip=infer['image_output_blip_global']
    text_feats_blip = infer['text_output_blip_global']
    image_feats_blip_background=infer['image_output_blip_background']
    text_feats_blip_background=infer['text_output_blip_background']
    image_feats_blip_entries=infer['image_output_blip_entries']
    text_feats_blip_entries=infer['text_output_blip_entries']
    #消融实验
    image=infer['text_output1']
    text=infer['image_output1']
    #loss_zero=compute_SCAN(image,text,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])

    logit_scale = pl_module.logit_scale
    '''logits_per_image = logit_scale * image @ text.t()
    logits_per_text = logit_scale * text @ image.t()
    batch_size = image.size(0)
    ground_truth = torch.arange(batch_size, device=image.device)
    loss_i2t = F.cross_entropy(logits_per_image, ground_truth)
    loss_t2i = F.cross_entropy(logits_per_text, ground_truth)
    loss_zero_clip = (loss_t2i + loss_i2t) / 2'''




    loss1=compute_SCAN(image_feats, text_feats, margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    loss2=compute_SCAN(image_feats_blip,text_feats_blip,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    loss3=compute_SCAN(image_feats_blip_background,text_feats_blip_background,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    loss4=compute_SCAN(image_feats_blip_entries,text_feats_blip_entries,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    ...
    loss_i2t = F.cross_entropy(logits_per_image, ground_truth)
    loss_t2i = F.cross_entropy(logits_per_text, ground_truth)
    loss5=(loss_t2i+loss_i2t)/2

    logits_per_image_blip = logit_scale * image_feats_blip @ text_feats_blip.t()
    logits_per_text_blip = logit_scale * text_feats_blip @ image_feats_blip.t()
    batch_size = image_feats.size(0)
    ground_truth = torch.arange(batch_size, device=image_feats_blip.device)
    loss_i2t_blip = F.cross_entropy(logits_per_image_blip, ground_truth)
    loss_t2i_blip = F.cross_entropy(logits_per_text_blip, ground_truth)
    loss6 = (loss_t2i_blip + loss_i2t_blip) / 2

    logits_per_image_blip_background = logit_scale * image_feats_blip_background @ text_feats_blip_background.t()
    logits_per_text_blip_background = logit_scale * text_feats_blip_background @ image_feats_blip_background.t()
    batch_size = image_feats_blip_background.size(0)
    ground_truth = torch.arange(batch_size, device=image_feats_blip_background.device)
    loss_i2t_blip_background = F.cross_entropy(logits_per_image_blip_background, ground_truth)
    loss_t2i_blip_background = F.cross_entropy(logits_per_text_blip_background, ground_truth)
    loss7 = (loss_t2i_blip_background + loss_i2t_blip_background) / 2

    logits_per_image_blip_entries = logit_scale * image_feats_blip_entries @ text_feats_blip_entries.t()
    logits_per_text_blip_entries = logit_scale * text_feats_blip_entries @ image_feats_blip_entries.t()
    batch_size = image_feats_blip_entries.size(0)
    ground_truth = torch.arange(batch_size, device=image_feats_blip_entries.device)
    loss_i2t_blip_entries = F.cross_entropy(logits_per_image_blip_entries, ground_truth)
    loss_t2i_blip_entries = F.cross_entropy(logits_per_text_blip_entries, ground_truth)
    loss8 = (loss_t2i_blip_entries + loss_i2t_blip_entries) / 2


    ret = {
        #"irtr_loss": loss1+loss2+loss3+loss4+loss5+loss6+loss7+loss8,
        #"irtr_loss": loss5 + loss6 + loss7 + loss8,
        "irtr_loss": loss1+loss2+loss3+loss4
        #"irtr_loss": loss1 83.3 70.2
        #"irtr_loss": loss1+loss2+loss5+loss6
        #"irtr_loss":loss_zero+loss_zero_clip
        #"irtr_loss": loss1+loss5+loss6
    }
...
def compute_irtr_val(pl_module):
    ...
    for (images,caption,image_blip_global_pixel_values, image_blip_global_input_ids, image_blip_global_attention_mask, image_blip_background_pixel_values, image_blip_background_input_ids, image_blip_background_attention_mask, image_blip_entries_pixel_values, image_blip_entries_input_ids, image_blip_entries_attention_mask,input_ids,ids) in val_dataloader:
        ...
        img_emb = infer['image_output_clip']
        cap_emb = infer['text_output_clip']
        """img_emb = infer['image_output1']
        cap_emb = infer['text_output1']"""
