...
class CocoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if is_clip:
            ...
            return image,caption,input_ids,index
        else:
            pass
...
class FlickrDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(root, path)).convert('RGB')
        if self.use_blip is True:
            qusetion_global = 'Question: What does this picture describe? Answer:'
            qusetion_background = 'Question: What semantic information is provided in the background of the image besides the subject in this image? Answer:'
            question_entries = 'Question: Please list in detail the physical objects contained in this picture Detailed description Answer:'
            ...
            if is_clip:
                ...
                return (image_trans,
                        caption,
                        image_blip_global_pixel_values,
                        image_blip_global_input_ids,
                        image_blip_global_attention_mask,
                        image_blip_background_pixel_values,
                        image_blip_background_input_ids,
                        image_blip_background_attention_mask,
                        image_blip_entries_pixel_values,
                        image_blip_entries_input_ids,
                        image_blip_entries_attention_mask,
                        input_ids,
                        index)
        else:
            if self.transform is not None:
            ...
            if is_clip:
                input_ids = convert_to_feature(caption, self.tokenizer)
                input_ids = torch.as_tensor(input_ids, dtype=torch.long)
                return image_trans, caption, input_ids, index
...
def collate_fn_bert(data):
    image,caption,input_ids,index=zip(*data)
    input_ids = torch.stack(input_ids, 0)
    input_ids = torch.squeeze(input_ids, dim=1)
    image = torch.stack(image, dim=0)
    ids = np.array(index)
    return image,caption,input_ids,ids

def collate_fn_bert_useBLIP2(data):
    (image,
    caption,
    image_blip_global_pixel_values,
    image_blip_global_input_ids,
    ...
    def train_dataloader(self):
        if self.use_blip2 is True:
            loader = torch.utils.data.DataLoader(dataset=self.train_dataset,
                                                 ...
                                                 collate_fn=collate_fn_bert_useBLIP2)
            return loader
        else:
            loader=torch.utils.data.DataLoader(dataset=self.train_dataset,
                                                batch_size=self.batch_size,
