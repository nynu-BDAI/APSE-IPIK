...
from .dist_utils_no_blip2 import all_gather
import torch.nn.functional as F
...
def compute_irtr_my(pl_module, batch):
    images,caption,input_ids,ids=batch
    ...
    #add
    # text_global=infer['text_output_blip_global']
    # text_background=infer['text_output_blip_background']
    # text_entries=infer['text_output_blip_entries']
    ...
    loss_triple=compute_SCAN(image_feats,text_feats,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    #add
    # loss_triple1=compute_SCAN(image_feats,text_global,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    # loss_triple2=compute_SCAN(image_feats,text_background,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])
    # loss_triple3=compute_SCAN(image_feats,text_entries,margin=pl_module.hparams.config["margin"], direction=pl_module.hparams.config["direction"])

    ret = {
        #"irtr_loss": loss_clip+ loss_triple+loss_triple1+loss_triple2+0.001*loss_triple3
        "irtr_loss": loss_clip + loss_triple
    }

    return ret

def shard_xattn_i2t(images, captions, shard_size=125):
